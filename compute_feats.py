...
def main():
    ...
    parser.add_argument('--dataset', default='TCGA-lungcut-single', type=str, help='Dataset folder name [TCGA-lung-single]')#default='TCGA-lung-single'
    ...
    if args.magnification == 'tree' and args.weights_high != None and args.weights_low != None:
        ...
        if args.weights == 'ImageNet':
            ...
            if args.weights is not None:
                ...
                weight_path ='/home/user/big_data/Wangping/dsmil-wsi-master/weights/tcga_embedder.pth'
            state_dict_weights = torch.load(weight_path)
            for i in range(4):
                state_dict_weights.popitem()
    ...
    if args.magnification == 'tree' or args.magnification == 'low' or args.magnification == 'high' :
        ...
        bags_path = os.path.join('/home/user/big_data/from_root_user/user/tangdan/DS_MIL/datasets', args.dataset, '*', '*')
    ...
    if args.magnification == 'tree':
        compute_tree_feats(args, bags_list, i_classifier, i_classifier, feats_path, 'cat')
    else:
        compute_feats(args, bags_list, i_classifier, feats_path, args.magnification)
    n_classes = glob.glob(os.path.join('/home/user/big_data/from_root_user/user/tangdan/DS_MIL/datasets', args.dataset, '*'+os.path.sep))
