    ...
    for i in range(0, num_bags):
        ...
        with torch.no_grad():
            for iteration, batch in enumerate(dataloader):
                ...
                # single_predictions = milnet.s_classifier(feats, args.single_label)
            ...
            single_predictions = milnet.s_classifier(bag_feats, args.single_label)
            # print(single_predictions)
            
            bag_prediction, A, _ = milnet.b_classifier(bag_feats, ins_classes, single_predictions)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--thres', nargs='+', type=float, default=[0.3988])# 0.7371, 0.2752
    parser.add_argument('--class_name', nargs='+', type=str, default=None)
    parser.add_argument('--embedder_weights', type=str, default='/data_disk/hj/wp/copy/test-c16/weights/embedder.pth')
    parser.add_argument('--aggregator_weights', type=str, default='/data_disk/hj/wp/copy/weights/04282023/1.pth')
    parser.add_argument('--bag_path', type=str, default='test-c16/patches')
    parser.add_argument('--patch_ext', type=str, default='jpg')
    parser.add_argument('--map_path', type=str, default='test-c16/output')
    parser.add_argument('--export_scores', type=int, default=0)
    parser.add_argument('--score_path', type=str, default='test-c16/score')
    parser.add_argument('--alpha', type=float, default=0.15)
    parser.add_argument('--beta', type=float, default=0.50)
    parser.add_argument('--single_label', type=float, default=1)
    ...
    s_classifier = mil.SingleClassifier(args.feats_size, out_class=args.num_classes, alpha=args.alpha,beta=args.beta).cuda()
